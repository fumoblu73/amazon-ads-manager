...
// Funzione per ottenere il profile ID per un marketplace (legge env a runtime)
export const getProfileIdForMarketplace = (marketplace: string): string | null => {
  ...
  const profileId = profileIds[marketplaceUpper];
  console.log(`🔍 getProfileIdForMarketplace(${marketplace}): ${profileId || 'NOT FOUND'}`);
  return profileId || null;
};

// Legacy: MARKETPLACE_PROFILES per backward compatibility
export const MARKETPLACE_PROFILES: Record<string, string> = {
  get US() { return process.env.AMAZON_PROFILE_US || ''; },
  get CA() { return process.env.AMAZON_PROFILE_CA || ''; },
  get UK() { return process.env.AMAZON_PROFILE_UK || ''; },
  get GB() { return process.env.AMAZON_PROFILE_UK || ''; },
  get DE() { return process.env.AMAZON_PROFILE_DE || ''; },
  get FR() { return process.env.AMAZON_PROFILE_FR || ''; },
  get IT() { return process.env.AMAZON_PROFILE_IT || ''; },
  get ES() { return process.env.AMAZON_PROFILE_ES || ''; },
  get AU() { return process.env.AMAZON_PROFILE_AU || ''; }
};
